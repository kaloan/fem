...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Но през цялото време си затворихме очите, че условието на Дирихле не е хомогенно. Затова тук след всяка стъпка пресмятания, ще задаваме компонентите на $\mathbf{Q}_{j + 1}$, съотвестващи на възлите по границата $\Gamma_{D}$ да са $\sin(8\pi t_{j+1})$. За намирането на $C$ може отново да концентрираме $I + A$.
\end{large}
\end{document}
